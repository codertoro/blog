if (typeof myhkplayer !== 'undefined') {
	throw new Error('闊充箰鎾斁鍣ㄥ凡鍔犺浇锛屾棤闇€閲嶅娣诲姞');
}
if (typeof jQuery === 'undefined') {
	if (confirm("缃戠珯娌℃湁鍔犺浇jQuery鎻掍欢锛屾槸鍚︽煡鐪嬪浣曟坊鍔爅query.min.js锛焅n鎵惧埌銆恓query.min.js銆戯紝澶嶅埗<script>鏍囩锛屾坊鍔犲埌鎾斁鍣ㄤ唬鐮佷笂鏂瑰嵆鍙�")) {
		window.location = "https://www.bootcdn.cn/jquery/"
	} else {
		throw new Error('璇峰厛鍔犺浇jQuery鎻掍欢');
	}
}
...
if (navigator.userAgent.match(/(iPhone|iPod|iPad|Android|ios|Nokia|Black Berry|MIDP|Phone)/i) && mobiles === "no") {
	...
	if (myhkLoad && window.location.href.indexOf("music.mxbk.cc") < 0) {
		$("#myhkplayer").hide()
	} else {
		var songSheetList;
		var webURL = 'https://music.mxbk.cc/';
		var keyId = $("#xplayer").attr("key");
		console.log("Mx闊充箰鎾斁鍣ㄦ彃浠讹細" + webURL);
		...
		if (css(span, "font-family") === "FontAwesome") {
			console.log("鏈珯宸插瓨鍦‵ontAwesome锛屾挱鏀惧櫒鍙栨秷鍔犺浇")
		} else {
			console.log("鏈珯鏈壘鍒癋ontAwesome锛屾挱鏀惧櫒灏嗗姞杞�");
			$("head").append("<link rel='stylesheet' id='font-awesome-css'  href='https://cdn.staticfile.org/font-awesome/4.7.0/css/font-awesome.min.css' type='text/css' media='all' />")
		};
...
"			鎮ㄧ殑娴忚鍣ㄤ笉鏀寔canvas锛岃鏇存崲楂樼骇鐗堢殑娴忚鍣紒" + 
...
"                    class=\"fa fa-random current\"></i> 闅忔満鎾斁</span></div>" + 
...
"        </div>" + 
"        <div class=\"control\">" + 
"            <i class=\"aprev fa fa-fast-backward\" title=\"涓婁竴涓撹緫\"></i>" + 
"            <i class=\"prev fa fa-backward\" title=\"涓婁竴棣朶"></i>" + 
"            <div class=\"status\">" + 
"                <b>" + 
"                    <i class=\"play fa fa-play-circle\" title=\"鎾斁\"></i>" + 
"                    <i class=\"pause fa fa-pause-circle\" title=\"鏆傚仠\"></i>" + 
"                </b>" + 
...
"                </div>" + 
"            </div>" + 
"            <i class=\"next fa fa-forward\" title=\"涓嬩竴棣朶"></i>" + 
"            <i class=\"anext fa fa-fast-forward\" title=\"涓嬩竴涓撹緫\"></i>" + 
"        </div>" + 
...
"            </div>" + 
"            <div class=\"qhms\">" + 
"                <i class=\"fa fa-random\" title=\"闅忔満鎾斁\"></i>" + 
"            </div>" + 
...
"<div id=\"myhkLrc\"></div>");
		if (top.location != self.location) {
			...
			var cicleTime = null;
			var myhkadTime = null;
			$songName.html("<a style=\"color:#f00\">鍒濆鍖栨瓕鍗�</a>");
			$songFrom.html("<a style=\"color:#f00\">Mx</a>");
			$songFrom1.html("<a style=\"color:#f00\">闊充箰鎾斁鍣�</a>");
			$songFrom3.html("<i class=\"fa fa-smile-o\"></i> 娆㈣繋鍏変复");
			...
			var myhkMedia = {
				...
				error: function() {
					...
					$player.removeClass("playing");
					if (errCount > 2) {
						myhkTips.show("杩炵画鎾斁澶辫触瓒呰繃3娆★紝宸插仠姝㈡挱鏀�");
						errCount = 0;
						errjc = true
					} else {
						errCount++;
						errjc = false;
						myhkTips.show(songSheetList[albumId].songNames[songId] + " - 璧勬簮鑾峰彇澶辫触锛佸皾璇曡幏鍙栦笅涓€棣�...");
						console.log("鎾斁澶辫触 " + errCount + " 娆★紒");
						setTimeout(function() {
				...
				timeupdate: function() {
					...
					$.cookie("yimihua",audio.currentTime);
			...
			$(".pause", $player).click(function() {
				hasgeci = false;
				$("li", $albumList).eq(albumId).addClass(cur).find(".artist").html("鏆傚仠鎾斁 > ").parent().siblings().removeClass(cur).find(".artist").html("").parent();
				myhkTips.show("鏆傚仠鎾斁 - " + songSheetList[albumId].songNames[songId]);
				$cover.removeClass("coverplay");
				audio.pause();
			...
			$(".play", $player).click(function() {
				hasgeci = true;
				$("li", $albumList).eq(albumId).addClass(cur).find(".artist").html("褰撳墠鎾斁 > ").parent().siblings().removeClass(cur).find(".artist").html("").parent();
				startPlay();
			...
			$(".qhms", $player).click(function() {
				random ? (random = false, myhkTips.show("涓撹緫寰幆"), $(this).html("<i class = \"random fa fa-retweet\" title=\"涓撹緫寰幆\"></i>"), $songFrom2.html("<i class=\"fa fa-retweet current\"></i> 涓撹緫寰幆")) : (loop ? (random = true, loop = false, myhkTips.show("闅忔満鎾斁"), $(this).html("<i class = \"random fa fa-random\" title=\"闅忔満鎾斁\"></i>"), $songFrom2.html("<i class=\"fa fa-random current\"></i> 闅忔満鎾斁")) : (random = false, loop = true, myhkTips.show("鍗曟洸寰幆"), $(this).html("<i class = \"random fa fa-refresh\" title=\"鍗曟洸寰幆\"></i>"), $songFrom2.html("<i class=\"fa fa-refresh current\"></i> 鍗曟洸寰幆")));
				musicTooltip()
			});
			$(".random", $player).click(function() {
				...
				random = true;
				myhkTips.show("闅忔満鎾斁");
				$songFrom2.html("<i class=\"fa fa-random current\"></i> 闅忔満鎾斁");
			...
			$(".loop", $player).click(function() {
				...
				random = false;
				myhkTips.show("椤哄簭鎾斁");
				$songFrom2.html("<i class=\"fa fa-retweet\"></i> 椤哄簭鎾斁");
			...
			$Volumeprogress.Drag({
				...
				callback: function(a, b, c, e, g) {
					if (5 == arguments.length) {
						...
						setTimeout(function() {
							myhkTips.show("闊抽噺锛�" + parseInt(d * 100) + "%")
						}, 500)
					} else {
						visTsMoving = !1
					}
			...
			$(".volumeprogress .progressbg", $player).click(function(a) {
				...
				setTimeout(function() {
					myhkTips.show("闊抽噺锛�" + parseInt(a * 100) + "%")
				}, 500)
			});
			...
			$(".switch-ksclrc").click(function() {
				...
				if (!$("#myhkLrc").hasClass("hide") || !$("#myhkKsc").hasClass("hidePlayer")) {
					...
					myhkTips.show("姝岃瘝鏄剧ず宸插叧闂�");
					songFrom33 = "鍏抽棴";
					$songFrom4.html("<i class=\"fa fa-toggle-off\" title=\"鎵撳紑姝岃瘝\"></i>")
				};
				musicTooltip()
			});
			myhkplayer.playList = {
				creat: {
					album: function() {
						if (typeof myhkid === "undefined") {
							...
							$(".musicheader", $albumList).html("<a href=\"" + webURL + "\" title=\"娆㈣繋浣跨敤Mx闊充箰鎾斁鍣�</a>")
						};
						var b = songSheetList.length,
							albumList = "";
						for (var c = 0; c < b; c++) {
							albumList += "<li><i class=\"fa fa-angle-right\"></i><span class=\"index\">" + (c + 1) + "</span><span class=\"artist\"></span>銆�" + songSheetList[c].songSheetName + "銆� - " + songSheetList[c].author + "</li>"
						};
					...
					song: function(a, b) {
						...
						$("li", $songList).click(function() {
							...
							if ($(this).hasClass(cur)) {
								myhkTips.show("姝ｅ湪鎾斁 - " + songSheetList[albumId].songNames[songId].replace(songId + 1 + "#", ""))
							} else {
								$.cookie("auto_playre", "yes");
								songId = $(this).index();
								myhkMedia.getInfos(songId, albumId)
			...
			var myhkLrc = {
				...
				ksc: {
					format: function(b) {
						gcdw = true;
						if (ycgeci == true) {
							$songFrom3.html("<i class=\"fa fa-map-marker\"></i> 姝岃瘝瀹氫綅")
						};
						hasKsc = true;
						var c = [],
							kscEndTimeLine = [],
						...
						for (var i = 0; i < c.length; i++) {
							...
							if (kscCont[i].indexOf("鐢凤細") >= 0) {
								sex = "m";
								kscCont[i] = kscCont[i].replace("鐢凤細", " ")
							};
							if (kscCont[i].indexOf("濂筹細") >= 0) {
								sex = "g";
								kscCont[i] = kscCont[i].replace("濂筹細", " ")
							};
							if (kscCont[i].indexOf("鍚堬細") >= 0) {
								sex = "t";
								kscCont[i] = kscCont[i].replace("鍚堬細", " ")
							};
					...
					play: function() {
						...
						if ($(".myhkKsc" + (a + 10)).length && !$(".myhkKsc" + (a + 10)).hasClass(cur)) {
							if (ycgeci == true) {
								$songFrom3.html("<i class=\"fa fa-check-circle\"></i> KCS姝岃瘝");
								gcdw = false
							};
							if (hasKsc) {
								$("#myhkKsc").addClass("showPlayer")
			...
			$.ajax({
				...
				success: function() {
					if (typeof myhkid !== "undefined") {
						$(".myhkgeci", $player).html("<a style=\"float: right\" href=\"" + webURL + "\" title=\"Mx闊充箰鎾斁鍣╘" target=\"_blank\"><i class=\"fa fa-share-alt\"></i> 鍏嶈垂鐢宠</a>");
					...
					if ($.cookie("random_play") != null) {
						if ($.cookie("random_play") == "true") {
							$(".qhms", $player).html("<i class = \"random fa fa-random\" title=\"闅忔満鎾斁\"></i>");
							$songFrom2.html("<i class=\"fa fa-random\"></i> 闅忔満鎾斁");
							random = true
						} else {
							$(".qhms", $player).html("<i class = \"random fa fa-retweet\" title=\"涓撹緫寰幆\"></i>");
							$songFrom2.html("<i class=\"fa fa-retweet\"></i> 涓撹緫寰幆");
							random = false
						}
					} else {
						if (randomPlayer != 1) {
...
function netmusic() {
	...
	d.onerror = function() {
		...
		setTimeout(function() {
			myhkTips.show(songSheetList[albumId].songNames[songId] + " - 涓撹緫鍥剧墖鑾峰彇澶辫触")
		}, 4000)
	};
	$cover.html(d);
	if (background == 1) {
...
function startPlay() {
	...
	if (a) {
		a.then(() => {
			...
			$cover.addClass("coverplay");
			myhkTips.show("寮€濮嬩粠" + songFrom55 + "鎾斁 - " + songSheetList[albumId].songNames[songId]);
			if (showMsg === 1) {
			...
			console.log("褰撳墠鎾斁锛�" + songSheetList[albumId].songNames[songId] + " - " + songSheetList[albumId].artistNames[songId] + " 鏃堕暱锛�" + Math.floor(t / 60) + ":" + (t % 60 / 100).toFixed(2).slice(-2))
		}).catch((e) => {
			console.log("娴忚鍣ㄩ檺鍒堕煶棰戯紝璇锋墜鍔ㄧ偣鍑绘挱鏀撅紝涓嬫鏃犻渶鐐瑰嚮")
		})
	}
}
function allmusic() {
...
$(document).ready(function() {
	...
	audio.currentTime = parseFloat($.cookie("yimihua"));
})